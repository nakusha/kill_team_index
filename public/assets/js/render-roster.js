/*
 * 로스터 렌더러 — 내 로스터 화면과 대결 화면이 함께 쓴다.
 *
 * 담긴 내용(요원 인스턴스 · 고른 무기 · 장비)과 팀이 늘 가진 것(팩션 규칙 · 플로이)을
 * 한 팀 단위로 그려 준다. 저장은 하지 않는다 — 그리기만 한다.
 */
window.KTRender = (function () {
  "use strict";
  ...
  /** 행 수가 맞을 때만 한글표에 영문 이름을 짝지어 붙인다. */
   ...
   */
  function unitCard(operative, unit, index, total, options) {
  ...
  function factionRuleBlock(team, options) {
  ...
  function unitCards(units, options) {
    var seen = {};
    var totals = {};
    ...
    return units
      .map(function (row) {
        ...
        seen[opId] = index + 1;
        return unitCard(row.operative, row.unit, index, totals[opId], options);
      })
      .join("");
  }
  ...
  /**
   * 팀 하나를 통째로 그린다.
   * options.actions 가 true 면 편집·비우기 버튼을 붙인다(내 로스터에서만).
   * options.collapseRule 가 true 면 팩션 규칙을 접어 둔다(대결 화면처럼 좁을 때).
   */
  function teamSection(team, entry, options) {
    var opts = options || {};
    ...
    return (
      ...
      factionRuleBlock(team, opts) +
      ployGroups(team) +
      pickedList("장비", picked.equipment, equipmentEntry) +
      "</div>" +
      (picked.units.length
        ? '<p class="subhead">요원</p><div class="op-list">' +
          unitCards(picked.units, opts) +
          "</div>"
  ...
  /** 슬롯 하나(내 것 또는 상대 것)를 통째로 그린다. */
  function sideBody(store, options) {
    return store
      .teamIds()
      .map(function (teamId) {
        var team = KTX.findTeam(teamId);
        return team ? teamSection(team, store.teamEntry(teamId), options) : "";
      })
      .filter(Boolean)
      .join("");
  }
