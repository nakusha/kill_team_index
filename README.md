...
- **대전** — 로스터 둘을 나란히 놓고 본다(넓으면 반반, 좁으면 탭). 파일·붙여넣기로 주고받는다
...
| `public/versus.html`         | 대전 — 로스터 둘을 반반(좁으면 탭)으로 비교 |
...
- **주고받기**: 내보내기는 `{kind:"kt-index-roster", version:2, name, exportedAt, teams}`
  형태의 JSON 파일이다. 불러오면 **새 항목으로 추가**되며 기존 것을 덮어쓰지 않는다.
  불러온 값은 모양을 검사해 맞는 것만 남긴다(믿고 쓰지 않는다).
- **대전**(`versus.html`)은 좌·우에 각각 보관함 항목을 골라 올린다. 고른 조합은
  `kt-index:versus:v1` 에 남아 다음에 열어도 그대로다. 900px 이하에서는 탭으로 바뀐다.
