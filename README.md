...
- **내 로스터** — 요원(고른 무기)과 장비를 담고, 팩션 규칙·플로이는 팀 전부를 함께 본다
...
| `public/roster.html`         | 담은 요원(고른 무기)·장비 + 팩션 규칙·플로이 전체 |
...
킬팀 상세에서 요원을 담고(같은 요원을 여러 명 담을 수 있다), **요원마다 들 무기를
체크**하고, 팀 전용 장비를 골라 둔다. 저장은 브라우저 `localStorage` 한 곳 —
키는 `kt-index:roster:v2` 다.
...
```jsonc
{
  ...
  "teams": {
    "IMP-AOD": {
      "seq": 2, // uid 발급용 카운터
      "units": [
        // 요원 한 명 = 인스턴스 하나. 같은 요원이라도 무기를 따로 고른다
...
- 무기에는 ID 가 없어 **행 이름**을 키로 쓴다(한글표가 있으면 한글명). 원본 갱신으로
  이름이 바뀌면 로스터 화면에 "저장된 무기 n개가 지금 데이터에 없어 표시하지
  못했습니다" 를 띄운다. 조용히 삼키지 않는다.
- 같은 요원을 둘 이상 담으면 요원 카드에 **인스턴스 탭**(`1번째 · 2`)이 생기고,
  탭마다 무기 선택이 따로 저장된다.
- 무기를 고르지 않은 인스턴스는 로스터에서 전체 무기를 보여주고 `무기 미선택` 배지를
  붙인다. 팀 헤더에도 `무기 미선택 n명` 으로 모아 표시한다.
- **팩션 규칙과 플로이는 고르는 대상이 아니다.** 팩션 규칙은 팀에 늘 걸리고, 플로이는
  팀이 가진 것을 CP 만 내면 모두 쓸 수 있다. 그래서 저장하지 않고 로스터에서 **팀의
  전부를 그대로** 보여준다(팩션 규칙은 펼친 채, 플로이는 전략·화력전으로 나눠서).
  게임 중 이 화면 하나만 봐도 되게 요원 시트보다 위에 둔다.
- 서버로 보내지 않는다. 기기·브라우저가 바뀌면 따라오지 않는다.
- 저장소를 못 쓰는 환경(사생활 보호 모드 등)에서는 메모리로 물러나고, 화면에
  "저장 실패 — 이 창에서만 유지됩니다" 를 띄운다.
- 인원은 편성 정원(`size.total`)과 비교해 초과를 표시한다. 다만 요원 목록 안의
  선택 제약("↘ 1 … 중 선택")은 원본이 자연어라 **자동 검증하지 않는다.**
  대신 팀 상세의 편성 가이드 원문을 그대로 읽을 수 있게 링크해 둔다
  (근거는 `docs/plan/roster-loadout.md` — 요원 이름 매칭률 6%).
- v1(개수 맵) 데이터는 처음 읽을 때 v2 로 자동 변환한다. 인원은 보존되고 무기만
  미선택 상태가 되며, v1 키는 지우지 않고 남겨 둔다.
